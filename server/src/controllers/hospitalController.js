...
export async function dashboard(req,res){const h=await Hospital.findOne({ownerId:req.user._id});if(!h)throw new AppError('Hospital profile not found',404,'NOT_FOUND');const [active,pending,completed,ambulances]=await Promise.all([EmergencyRequest.countDocuments({hospitalId:h._id,status:{$nin:['COMPLETED','CANCELLED','REJECTED']}}),EmergencyRequest.countDocuments({notifiedHospitalIds:h._id,status:'HOSPITAL_NOTIFIED'}),EmergencyRequest.countDocuments({hospitalId:h._id,status:'COMPLETED'}),Ambulance.find({hospitalId:h._id})]);const requests=await EmergencyRequest.find({$or:[{hospitalId:h._id},{notifiedHospitalIds:h._id}]}).sort({priority:1,createdAt:-1}).limit(30).populate('patientId','name phone');ok(res,{hospital:h,metrics:{active,pending,completed,availableBeds:h.availableBeds,icuBeds:h.icuAvailable?1:0,availableAmbulances:ambulances.filter(a=>a.status==='AVAILABLE').length},requests,ambulances});}
